...
커맨드 모드(esc)에서
1. 'q'를 누르고 a~z 사이 문자로 단축키 설정하여 매크로 recording 시작.
...
- 편집 모드에서 ctrl + r, ctrl + r, 매크로 문자 를 순서대로 입력하면 그대로 출력된다.
- 커맨드 모드에서 "매크로문자p 를 입력하여 래지스터로부터 바로 붙여넣기 한다. (ex: "ap, "bp, "cp, "dp, ...)
 
